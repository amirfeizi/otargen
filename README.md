...
### Target safety check (by Ensembl ID)

`targetLiability()` bundles genetic constraint, known safety liabilities, and
DepMap essentiality into a single 0–1 safety score with a written rationale,
flagging genes that look risky for full inhibition or knockout versus ones that
may tolerate a partial or tissue-restricted approach.

```r
# Combined safety score + rationale for a target (TP53)
