...
#'
#' This function queries the Open Targets Genetics GraphQL API to retrieve DepMap essentiality data
#' for a specified gene.
#'
#' @param ensgId Character: ENSEMBL ID of the target gene (e.g., ENSG00000141510).
#'
#' @return Returns a tibble containing DepMap essentiality data for the specified gene.
#' @examples
#' \dontrun{
...
depMapQuery <- function(ensgId) {
  ...
  tryCatch({
    ...
    query <- "query DepmapQuery($ensgId: String!) {
    ...
    qry$query(name = "getDepMapData", x = query)
    
    cli::cli_progress_step(paste0("Downloading data for ENSEMBL ID: ", ensgId, " ..."), spinner = TRUE)
    
    # Execute the query
    output0 <- con$exec(qry$queries$getDepMapData, variables)
    output1 <- jsonlite::fromJSON(output0, flatten = TRUE)
    
    if (length(output1$data$target$depMapEssentiality) != 0) {
      final_output <- tibble::as_tibble(output1$data$target$depMapEssentiality)
      return(final_output)
    } else {
