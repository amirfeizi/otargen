...
`Otargen` simplifies the process of accessing data from [Open Targets Genetics](https://genetics.opentargets.org/). The data primarily revolves around 3 key components: study id, variant id and gene id. Given below are a few examples on how to retrieve and plot data using the package.

## Examples

\#**1. Retrieve colocalisation data for a list of *genes*:**
...
**3. Retrieves L2G model summary data for a *gene*.** The output can be further filtered by providing *l2g* score, *pvalue* and *vtype*. *vtype* refers to the mostSevereConsequence of a variant on the associated gene.
...
**6. Plot data obtained from the *PheWAS* function.** The PheWAS results are filtered for diseases.
...
**7. Plot the scores obtained from the *studiesAndLeadVariantsForGeneByL2G* function and filter it for a specific *disease*.**
